...
Options for generating the HTML report:
    ...
    -c, --column            Column name used (e.g., viterbi)
...
if [[ -z "${NAME}" || -z "${OUTPUT_DIR}" || -z "${COLUMN}" ]]; then
    echo "Error: Missing required arguments (--name, --output-dir, and --column are required)." >&2
    show_help
    exit 1
fi
