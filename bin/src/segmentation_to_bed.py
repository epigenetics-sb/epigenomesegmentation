...
def combine_data(model, input, index, columns, state_column):
    ...
    if 'coverage_marker' in model:
        combined.append(pd.read_csv(model['coverage_data'][index], sep='\t'))
...
def to_bed_methylation(model, output, sequence, column):
    data = pd.read_csv(model['coverage_data'][0], sep='\t', converters={0: str})
    ...
    prefix = os.path.basename(model['coverage_data'][0])
