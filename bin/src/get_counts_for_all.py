...
if 'coverage_marker' in marker:
    ...
    if len(files) != len(cov_files):
        raise Exception('Number of histone count samples and coverage data samples must correspond!')
...
for i in range(len(files)):
    ...
    if len(cov_files) > 0:
        methData = pd.read_csv(cov_files[i], sep='\t')
        methData.to_csv(out_dir + "/coverage_data_" + filename, sep='\t', header=False, index=False)
    else:
        with open(out_dir + "/coverage_data_" + filename, 'w') as _:
            pass
