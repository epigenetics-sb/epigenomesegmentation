    ...
    try
    {
        ...
        if (!viterbiPath.empty())
        {
            ...
            for (size_t k = 0; k < startIndex.size(); ++k)
            {
                ...
                std::pair<double, std::vector<int>> decoding = hmm.viterbi_decoding(std::make_pair(startIndex[k], end), observation, nObservation);
                hmm.process_state_sequence(decoding.second);
        ...
        if (!posteriorDecoding.empty())
        {
            ...
            {
                size_t end = k < startIndex.size()-1 ? startIndex[k+1] : rows;
                std::vector<std::pair<int, double>> decoding = hmm.posterior_decoding(std::make_pair(startIndex[k], end), observation, nObservation);
                std::vector<int> stateSequence;
                std::transform(decoding.begin(), decoding.end(), std::back_inserter(stateSequence), [] (std::pair<int, double>& x) {return x.first;});
                hmm.process_state_sequence(stateSequence);
                for (size_t i = 0; i < stateSequence.size(); ++i)
                {
                    outputDecoding << stateSequence[i] << '\t' << decoding[i].second << '\n';
                }
            }
            outputDecoding.close();
        }
