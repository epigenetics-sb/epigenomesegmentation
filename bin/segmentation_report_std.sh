#!/usr/bin/env bash
while getopts "n:o:i:c:" OPTION; do
    case $OPTION in
        n) name="$OPTARG" ;;
        o) output_dir=$OPTARG ;;
...
cat << EOF > "$output_file"
<!DOCTYPE html><html><head><title>STD Report - $name</title>$STYLE</head>
...
<div class="section"><h2>Normalized Counts ($column)</h2><img src="${name}-normEmission-${column}.png"></div>
...
<div class="card"><span class="label">Average Length</span><img src="${name}-stateLength-${column}.png"></div>
<div class="card"><span class="label">Coverage</span><img src="${name}-stateMembership-${column}.png"></div>
</div></div>
EOF

if [[ "${plot_statistics}" != "False" && "${plot_statistics}" != "false" ]]; then
cat << EOF >> "$output_file"
<hr><div class="section"><h2>Emission Distribution</h2><img src="${name}-stateDistribution-${column}.png"></div>
